...
filename = "trialsitting_11_02_21.txt"
...
def eval_performance(des_robotaction, detected_emgtrigger):
    ...
    for i in range(0, len(t_cueend)):
        ...
        if i == len(t_cueend)-1:
            ...
            if triggers_in_interval.size == 1:
                ...
                t_correct_emg = np.append(
                    t_correct_emg, t_emgtrigger[triggers_in_interval])

            if triggers_in_interval.size > 1:
                nbfalsetriggers_int = len(triggers_in_interval) - 1
                nb_falsetriggers = nb_falsetriggers + nbfalsetriggers_int
                t_correct_emg = np.append(
                    t_correct_emg, t_emgtrigger[triggers_in_interval[0]])
                t_usedaudio = np.append(t_usedaudio, t_cueend[i])
        ...
        if 0 <= i < len(t_cueend)-1:
            ...
            triggers_in_interval = np.where(np.logical_and(
                t_emgtrigger >= a, t_emgtrigger < b))[0]
            ...
            if triggers_in_interval.size > 1:
                ...
                t_correct_emg = np.append(
                    t_correct_emg, t_emgtrigger[first_trigger_idx])
    ...
    CRate = np.float(len(t_correct_emg)) / len(t_cuestart) * 100
...
start_tdelay, start_CR, start_falsetrig, start_failedtrig = eval_performance(
    y_start, startRobotMotion)
print('pauseRobotMotion: ')
pause_tdelay, pause_CR, pause_falsetrig, pause_failedtrig = eval_performance(
    y_pause, pauseRobotMotion)
print('activategripper: ')
grasp_tdelay, grasp_CR, grasp_falsetrig, grasp_failedtrig = eval_performance(
    y_grasp, activategripper)


# cannot plot the 3 like that because they have different lengths

# plot results
tdelay_data = [start_tdelay.tolist(), pause_tdelay.tolist(),
               grasp_tdelay.tolist()]
...
ax.set_xticklabels(['Start\nN=' + str(len(start_tdelay)), 'Pause\nN=' +
                    str(len(pause_tdelay)), 'Grasp\nN=' + str(len(grasp_tdelay))])
labelsize = 7
plt.rcParams['xtick.labelsize'] = labelsize
plt.rcParams['ytick.labelsize'] = labelsize

plt.title(
    "Time delay from EMG trigger detection to Robot activation [s]", fontsize=8)
